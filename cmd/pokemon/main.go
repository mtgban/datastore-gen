...
func promoTypesOf(s *single, p published, onShelf bool, own map[string]bool, finish, mark string) (kept, left []string, year, found, spoken string) {
	...
	for _, q := range s.quals {
		...
		if saysFinish(emit.PromoSlug(q.text), finish) {
			left = append(left, lowered)
			if found == "" && mark == "" {
				found = lowered
			}
		...
		if variantOnlyQuals[lowered] {
			...
			out = append(out, emit.PromoSlug(q.text))
			continue
		}
		...
		// never happened.
		if len(emit.PromoSlug(text)) > promoTypeLimit && namesASet(text, setNames) {
		...
		if stem, rest := publishedPrefix(text, pokemon, setNames); rest != "" &&
			(len(emit.PromoSlug(text)) > promoTypeLimit || p.tails[rest] || strings.EqualFold(rest, "Promo")) {
		...
		out = append(out, emit.PromoSlug(text))
	}
...
func mintFromCardmarket(path string, cards []any, sets map[string]any, ptcg []pokemontcgSet) ([]any, int, int) {
	...
	for _, id := range ids {
		...
		entry := map[string]any{
			"id":      fmt.Sprintf("%s_mkm%d%s", sanitizeID(number+"-"+total), id, emit.FinishSuffix(finish)),
			"name":    name,
			"setCode": code,
			"number":  number,
			"rarity":  rarity,
...
func main() {
	...
	for i := range singles {
		for _, q := range singles[i].quals {
			if len(emit.PromoSlug(q.text)) <= promoTypeLimit {
				continue
			}
	...
	for _, card := range mintable {
		...
		for _, finish := range finishes {
			entry := map[string]any{
				"id":       mintedIDBase(card.ID) + emit.FinishSuffix(finish),
	...
	for _, entry := range cards {
		...
		links["tcgdexId"] = id
		delete(item, "tcgdexId")
		linked++
	}
	log.Printf("external links: %d cards carry their tcgdexId under externalLinks", linked)
